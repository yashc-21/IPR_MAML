...
def train_and_evaluate(model,
    ...
    with tqdm(total=params.num_episodes) as t:
        for episode in range(params.num_episodes):
            ...
            adapted_state_dicts = []
            dataloaders_list = []
            init_weights = OrderedDict(model.named_parameters())
            init_weights = {k: v.clone() for k, v in init_weights.items()}
            for n_task in range(num_inner_tasks):
                task = task_type(meta_train_classes, num_classes, num_samples,
                                 num_query)
                dataloaders = fetch_dataloaders(['train', 'test'],
                                                task,params)
            ...
            for n_task in range(num_inner_tasks):
                ...
                net_clone.load_state_dict(a_dict)

                for p, target_p in zip(model.parameters(), net_clone.parameters()):
                    if p.grad is None:
                        if params.cuda:
                            p.grad = Variable(torch.zeros(p.size())).cuda()
                        else:
                            p.grad = Variable(torch.zeros(p.size()))
                    p.grad.data.add_((p.data - target_p.data)/float(num_inner_tasks))
